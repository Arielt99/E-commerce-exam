...
class AdminBrandController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $brand->save();
            return response()->json(['error' => false,'message' => 'Ajout de la marque avec succès.', 200]);
        }
        else {
            return response()->json([
                'error' => true, 'message' => 'Tous les champs sont obligatoire sauf description, verifier que les données renseignées soient corrects et que le champs << Nom >> soit unique.',404
            ]);
        }
    }
